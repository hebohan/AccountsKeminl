        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (context.Request.QueryString["type"] == "edit")//获取要修改的菜单
            {
                ...
                if (model != null)
                {
                    ...
                    List<Accounts.Model.Tb_Navigation> list = bll.GetModelList(" ParentId="+model.ParentId);
                    string str = "";
            ...
            {
                Accounts.BLL.Tb_Navigation bll = new Accounts.BLL.Tb_Navigation();
                List<Accounts.Model.Tb_Navigation> list = bll.GetModelList(" ParentId=0");
                sb.Append("<option value='0'></option>");
                foreach (Accounts.Model.Tb_Navigation model in list)
                {
            ...
            else if (context.Request.QueryString["type"] == "child")//获取所有子节点数量并生成排序选项
            {
                int Id = int.Parse(context.Request.QueryString["Id"]);
                Accounts.BLL.Tb_Navigation bll = new Accounts.BLL.Tb_Navigation();
                List<Accounts.Model.Tb_Navigation> list = bll.GetModelList(" ParentId="+Id);
                int i = 1;
                for ( ; i <= list.Count; i++)
                {
            ...
            else if (context.Request.QueryString["type"] == "save")//保存修改的菜单
            {
                ...
                model.Icon = icon;
                List<Accounts.Model.Tb_Navigation> list = bll.GetModelList(" ParentId=" + Parent);
