    ...
    public class EditRoles : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (context.Request.QueryString["type"] == "edit")//获取角色信息
            {
                ...
                Accounts.Model.Tb_Roles model= bll.GetModel(Id);
                StringBuilder sb = new StringBuilder();
                sb.Append(model.RolesName+",");
                sb.Append(model.Remark+",");
                Accounts.BLL.View_Users ubll = new Accounts.BLL.View_Users();
                List<Accounts.Model.View_Users> list = ubll.GetModelList(" Userid in(select UserId from Tb_RolesAddUser where RolesId="+Id+")");
                foreach(Accounts.Model.View_Users item in list)
                {
            ...
            else if (context.Request.QueryString["type"] == "getUser")//获取未绑定到当前角色的用户
            {
                StringBuilder sb = new StringBuilder();
                Accounts.BLL.View_Users ubll = new Accounts.BLL.View_Users();
                List<Accounts.Model.View_Users> list = new List<Accounts.Model.View_Users>();
            ...
            else if (context.Request.QueryString["type"] == "add")//添加
            {
                string name = context.Request.QueryString["name"];
                string remark = context.Request.QueryString["remark"];
                Accounts.Model.Tb_Roles model = new Accounts.Model.Tb_Roles();
                Accounts.BLL.Tb_Roles bll = new Accounts.BLL.Tb_Roles();
                model.Remark = remark;
                model.RolesName = name;
                if(bll.Add(model)>0)
            ...
            {
                int Id = int.Parse(context.Request.QueryString["Id"]);
                Accounts.BLL.Tb_RolesAndNavigation bll = new Accounts.BLL.Tb_RolesAndNavigation();
                List<Accounts.Model.Tb_RolesAndNavigation> list = new List<Accounts.Model.Tb_RolesAndNavigation>();
                list = bll.GetModelList(" RolesId=" + Id);
                StringBuilder sb = new StringBuilder();
                foreach (Accounts.Model.Tb_RolesAndNavigation model in list)
                {
            ...
            else if (context.Request.QueryString["type"] == "saveDistri")
            {
                ...
                foreach (string ss in str)
                {
                    ...
                    Accounts.Model.Tb_Navigation model = bll.GetModel(int.Parse(ss));
                    if (model.ParentId != 0)
                    {
