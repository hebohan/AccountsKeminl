...
function jprint(msgtitle, url, msgcss, callback) {
    var iconurl = "";
    switch (msgcss) {
        case "Success":
...
function Star(id, hid) {
    ...
    for (i = 0; i < this.oLi.length; i++) {
        this.oLi[i].index = i;
        if (i <= (out - 1)) {
            this.oLi[i].className = 'hove';
        }
        if (i==(out-1)) {
            this.oSpan.innerHTML = '<span class="c"><b>' + (this.oLi[i].index + 1) + '</b>分&nbsp;' + this.apl[this.oLi[i].index] + '</span>&nbsp;' + this.aMsg[this.oLi[i].index];
        }
