...
scheduler._temp_limit_scope = function () {
    ...
    function (e, t, s, r) {
        function i(e, t) {
            ...
            return e.valueOf() > r.valueOf() || i <= s.valueOf()
        }
        return scheduler.config.limit_view && (r = r || t, s = s || e, i(r, s) && t.valueOf() != r.valueOf()) ? (setTimeout(function () {
            var e = i(t, s) ? scheduler.config.limit_start : t;
            scheduler.setCurrentView(i(e, s) ? null : e, s)
        },
        1), !1) : !0
    }),
    scheduler.checkInMarkedTimespan = function (t, s, i) {
        ...
        n = new Date(t.start_date.valueOf()), d = scheduler.date.add(n, 1, "day"), l = scheduler._marked_timespans; n < t.end_date; n = scheduler.date.date_part(d), d = scheduler.date.add(n, 1, "day")) {
            ...
            if (_) for (var c = 0; c < _.length; c += 2) {
                ...
                f = _[c],
                g = _[c + 1];
                if (v > f && g > u && (a = "function" == typeof i ? i(t, u, v, f, g) : !1, !a)) break
            }
        }
        return !a
    };
    var n = scheduler.checkLimitViolation = function (e) {
        ...
        o = 0; o < i.length; o++) {
            ...
            h = r.limit_start && r.limit_end ? d.start_date.valueOf() >= r.limit_start.valueOf() && d.end_date.valueOf() <= r.limit_end.valueOf() : !0,
            h && (h = !scheduler.checkInMarkedTimespan(d, s,
            function (e, s, r, i, a) {
                var n = !0;
                return a >= s && s >= i && ((1440 == a || a > r) && (n = !1), e._timed && t._drag_id && "new-size" == t._drag_mode ? (e.start_date.setHours(0), e.start_date.setMinutes(a)) : n = !1),
                (r >= i && a > r || i > s && r > a) && (e._timed && t._drag_id && "new-size" == t._drag_mode ? (e.end_date.setHours(0), e.end_date.setMinutes(i)) : n = !1),
                n
            })),
            h || (h = t.checkEvent("onLimitViolation") ? t.callEvent("onLimitViolation", [d.id, d]) : h),
            l = l && h
        }
    ...
    scheduler._append_mark_now = function (e, t) {
        ...
        if (!this._table_view) {
            ...
            return i.days = e,
            scheduler._render_marked_timespan(i, null, e)
        }
    ...
    scheduler._render_marked_timespan = function (e, t, s) {
        ...
        var h = e.zones,
        _ = scheduler._get_css_classes_by_config(e);
        if (scheduler._table_view && "month" == scheduler._mode) {
            ...
            for (var v = 0; v < c.length; v++) {
                t = c[v],
                s = u[v];
                var f = Math.floor((this._correct_shift(s, 1) - a.valueOf()) / (864e5 * this._cols.length)),
                ...
                if (!this._ignores[g]) {
                    ...
                    for (var w = 0; w < h.length; w += 2) {
                        ...
                        k.style.width = (Math.round((D - E) / 1440 * x * Math.pow(10, 1)) / Math.pow(10, 1)) + "px",
                        t.appendChild(k),
                        r.push(k)
                    }
                }
            ...
            if (this._props && this._props[this._mode] && e.sections && e.sections[this._mode]) {
                ...
                M = this._get_section_sday(e.sections[this._mode]),
                N.size && M > N.position + N.size && (M = 0)
            }
            t = t ? t : scheduler.locate_holder(M);
            for (var v = 0; v < h.length; v += 2) {
                var E = Math.max(h[v], 60 * i.first_hour),
                D = Math.min(h[v + 1], 60 * i.last_hour);
                if (E >= D) {
                    if (v + 2 < h.length) continue;
                    return []
                }
                ...
                O = 36e5;
                k.style.top = Math.round((60 * E * 1e3 - this.config.first_hour * O) * this.config.hour_size_px / O) % L + "px",
                k.style.lineHeight = k.style.height = Math.max(Math.round(60 * (D - E) * 1e3 * this.config.hour_size_px / O) % L, 1) + "px",
                t.appendChild(k),
                r.push(k)
            }
        }
    ...
    scheduler.markTimespan = function (e) {
        ...
        if (t.length) {
            for (var s = [], r = 0; r < t.length; r++) {
                var i = t[r],
                a = scheduler._render_marked_timespan(i, null, null);
    ...
    scheduler.addMarkedTimespan = function (e) {
        ...
        if (t.length) {
            ...
            a = scheduler._marked_timespans_ids;
            a[r] || (a[r] = []);
            for (var n = 0; n < t.length; n++) {
                ...
                if (d.id = r, o) {
                    for (var _ in o) if (o.hasOwnProperty(_)) {
                        ...
                        u[c] || (u[c] = {}),
                        u[c][l] || (u[c][l] = {}),
                        u[c][l][h] || (u[c][l][h] = [], scheduler._marked_timespans_types || (scheduler._marked_timespans_types = {}), scheduler._marked_timespans_types[h] || (scheduler._marked_timespans_types[h] = !0));
                        var v = u[c][l][h];
                        d._array = v,
                        v.push(d),
                        a[r].push(d)
                    ...
                    scheduler._marked_timespans_types[h] || (scheduler._marked_timespans_types[h] = !0);
                    var v = i[s][l][h];
                    d._array = v,
                    v.push(d),
                    a[r].push(d)
    ...
    scheduler._add_timespan_zones = function (e, t) {
        ...
        for (var r = 0; r < s.length; r += 2) for (var i = s[r], a = s[r + 1], n = r + 2 == s.length, d = 0; d < t.length; d += 2) {
            var l = t[d],
            o = t[d + 1];
            if (o > a && a >= l || i > l && o >= i) s[r] = Math.min(i, l),
            s[r + 1] = Math.max(a, o),
            r -= 2;
            else {
                if (!n) continue;
    ...
    scheduler._subtract_timespan_zones = function (e, t) {
        for (var s = e.slice(), r = 0; r < s.length; r += 2) for (var i = s[r], a = s[r + 1], n = 0; n < t.length; n += 2) {
            ...
            if (l > i && a > d) {
                var o = !1;
                i >= d && l >= a && s.splice(r, 2),
                d > i && (s.splice(r, 2, i, d), o = !0),
                a > l && s.splice(o ? r + 2 : r, o ? 0 : 2, l, a),
                r -= 2;
                break
            }
        }
    ...
    scheduler._delete_marked_timespan_by_config = function (e) {
        ...
        for (var o = 0; o < n.length; o++) {
            var h = n[o],
            _ = scheduler._subtract_timespan_zones(h.zones, e.zones);
            if (_.length) h.zones = _;
            else {
                n.splice(o, 1),
                o--;
        ...
        if (!scheduler._table_view || "month" == scheduler._mode) {
            ...
            if (this._props && this._props[i]) {
                ...
                if (t = scheduler.date.date_part(new Date(this._date)), s = t.getDay(), r = t.valueOf(), a[i] && a[i][h.key]) {
                    var _ = a[i][h.key],
                    c = scheduler._get_types_to_render(_[s], _[r]);
            ...
            for (var f = 0; f < n.length; f++) scheduler._render_marked_timespan(n[f], e, t)
        }
    }),
