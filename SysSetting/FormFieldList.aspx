<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="FormFieldList.aspx.cs" Inherits="Accounts.Web.SysSetting.FormFieldList" %>
...
<head runat="server">
    ...
    <script language="javascript" type="text/javascript">
        function OpenNewForm() {
            location.href = "FormFieldDetail.aspx";
        }
        ...
        function OpenNewEdit() {
            if (hasChecked(document.all("ListData")) && isCheckOne(document.all("ListData"))) {
                ...
                if (issys == 1) {
                    $.dialog.alert("不能修改系统内置的表单字段！");
                    return;
                }
                location.href = "FormFieldDetail.aspx?id=" + getCheckVal("chkbox").split('|')[0];
            }
            else {
        ...
        function DelCheck() {
            if (hasChecked(document.all("ListData"))) {
                ...
                if (issys == 1) {
                    $.dialog.alert("不能删除系统内置的表单字段！");
                    return;
                }
                 $.dialog.confirm("删除后相应表单上的数据也会删除，请确定是否删除该条记录?", function () {
        ...
        <div id="MainCont">
            ...
            <cc1:WebGridView CssClass="ListTable hei12" ID="ListData" runat="server" AutoGenerateColumns="False"
                ...
                <Columns>
                    ...
                    <ItemTemplate >
                        <%#  DataBinder.Eval(Container,"DataItem.is_sys").ToString() == "是"?  DataBinder.Eval(Container,"DataItem.name") :"<a href='FormFieldDetail.aspx?id="+DataBinder.Eval(Container,"DataItem.id") +"'>" +DataBinder.Eval(Container,"DataItem.name") + "</a>"%>
                    </ItemTemplate>
                    </asp:TemplateField>
                    ...
                    <ItemStyle Wrap="False" Width="20%"  HorizontalAlign="Center" />
                    <ItemTemplate >
                        <%# GetTypeCn(DataBinder.Eval(Container,"DataItem.control_type").ToString())%>
                    </ItemTemplate>
                    </asp:TemplateField>
                    <asp:TemplateField HeaderText="排序号">
                        ...
                        <ItemTemplate>
                            <%#DataBinder.Eval(Container,"DataItem.is_sys").ToString() == "是"? "系统排序":DataBinder.Eval(Container,"DataItem.sort_id").ToString()%>
                        </ItemTemplate>
                    </asp:TemplateField>
                    <asp:BoundField DataField="is_required" HeaderText="必填">
