...
lhgdialog.setting =
{
	...
	max: true,                  // 是否显示最大化按钮
	min: true,                  // 是否显示最小化按钮
	zIndex: 9976,				// 对话框叠加高度值(重要：此值不能超过浏览器最大限制)
...
lhgdialog.friendrequest = function (sendUrl) {
    var input;
    ...
    return lhgdialog({
        ...
        lock: true,
        parent: parent || null,
        content: '<div class="request">验证消息：<br/><textarea id="message" class="message"></textarea></div>',
        init: function () {
            input = this.DOM.content[0].getElementsByTagName('input')[0];
            input.select();
            input.focus();
        ...
        ok: function (here) {
            $.ajax({
                type: "POST",
                url: sendUrl + "&message=" + $("#message").val(),
                dataType: "json",
                timeout: 20000,
