...
this.width =  "auto";
this.height = "auto";
this.bodyStyle = 'background-color: #FFFFFF; font-family: "宋体"; font-size: 12px;';
this.imgURL = _editor_url + '../../images/edit/';
this.debug  = 0;
...
this.btnList = {
    ...
    "outdent":        ['Outdent',              '左缩进',    'editor_action(this.id)',  'ed_indent_less.gif'],
    "indent":         ['Indent',               '右缩进',    'editor_action(this.id)',  'ed_indent_more.gif'],
    ...
    "horizontalrule": ['InsertHorizontalRule', '水平线',    'editor_action(this.id)',  'ed_hr.gif'],
    "createlink":     ['CreateLink',           '超连接',    'editor_action(this.id)',  'ed_link.gif'],
    ...
    "htmlmode":       ['HtmlMode',             '编辑HTML',   'editor_setmode(\''+objname+'\')', 'ed_html.gif'],
    "popupeditor":    ['popupeditor',          '新窗口中编辑',     '',  'fullscreen_maximize.gif'],
    "about":          ['about',                '版权说明',  '',  'ed_about.gif'],
...
function editor_generate(objname,userConfig) {
  ...
  for (var btnGroup in config.toolbar) {
    ...
    for (var btnItem in config.toolbar[btnGroup]) {
      ...
      // fontname
      if (btnName == "fontname") {
        toolbar += '<select id="_' +objname+ '_FontName" style="width:100px" onChange="editor_action(this.id)" unselectable="on" style="margin: 1 2 0 2; font-size: 12px;">';
      ...
      // fontsize
      if (btnName == "fontsize") {
        toolbar += '<select id="_' +objname+ '_FontSize" style="width:80px" onChange="editor_action(this.id)" unselectable="on" style="margin: 1 2 0 0; font-size: 12px;">';
  ...
  + '</td></tr></table>\n'
  + '</td></tr></table></span>\n'
  + '<textarea ID="_' +objname + '_editor" style="border-left:#CCCCFF 1px solid; border-right:#CCCCFF 1px solid; border-top:#CCCCFF 1px solid; border-bottom:#CCCCFF 1px solid;width:' +config.width+ '; height:' +config.height+ '; margin-top: -1px; margin-bottom: -1px;" wrap=soft></textarea>';
...
function editor_action(button_id) {
  ...
  // help popup
  if (cmdID == 'showhelp') {
    window.open(_editor_url + "../../Includes/popups/editor_help.html", 'EditorHelp');
    return;
  }

  // popup editor
  if (cmdID == 'popupeditor') {
    window.open(_editor_url + "../../Includes/popups/fullscreen.html?"+objname,
                'FullScreen',
  ...
  // Custom2
    ...
    var myText = showModalDialog(_editor_url + "../../Includes/popups/custom2.html",
  ...
  // fgColor and bgColor
    ...
    var newcolor = showModalDialog(_editor_url + "../../Includes/popups/select_color.html", oldcolor, "resizable: no; help: no; status: no; scroll: no;");
  ...
  // commandID and pass it to execCommand().   See http://msdn.microsoft.com/workshop/author/dhtml/reference/commandids.asp
    ...
    // insert image
      ...
      showModalDialog(_editor_url + "../../Includes/popups/insert_image.html", editdoc, "resizable: no; help: no; status: no; scroll: no; ");
    }

    // insert table
    else if (cmdID.toLowerCase() == 'inserttable'){
      showModalDialog(_editor_url + "../../Includes/popups/insert_table.html?"+objname,
                                 window,
                                 "resizable: yes; help: no; status: no; scroll: no; ");
    }
...
function editor_setmode(objname, mode) {
 ...
 // src="' +_editor_url+ '../../Includes/popups/blank.html"
...
function editor_about(objname) {
  showModalDialog(_editor_url + "../../Includes/popups/about.html", window, "resizable: yes; help: no; status: no; scroll: no; ");
}
