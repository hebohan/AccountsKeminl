...
strFrame+='<span id=tmpSelectHourLayer  style="z-index: 9999;position: absolute;top: 188; left: 77px;display: none"></span>';
strFrame+='<span id=tmpSelectMinuteLayer style="z-index:9999;position:absolute;top: 188; left: 119px;display: none"></span>';
...
strFrame+='          <tr bgcolor="#63A3E9"><td id=bUseTimeLayer width=72  style="cursor:hand" title="点击这里启用/禁用时间"';
...
function setday(tt,t) //主调函数
{
    if(tt.disabled) return; //如果日期文本框不能用,不往下执行,EditBy:LPF
	...
	//begin
	//时间层的定位,2009-04-13,hw.lee
	try
	{
	    var tempDateLayer = document.getElementById("endDateLayer");
	    while (tempDateLayer = tempDateLayer.offsetParent)
	    {
	        if(tempDateLayer.tagName == "TD" || tempDateLayer.style.position == "absolute" || tempDateLayer.style.position == "relative")
	        {
	            break;
	        }
	    }
	    if(tempDateLayer != null)
	    {
	        var tdltop = tempDateLayer.offsetTop;
	        var tdlleft = tempDateLayer.offsetLeft;
	        while(tempDateLayer = tempDateLayer.offsetParent)
            {
                tdltop += tempDateLayer.offsetTop;
                tdlleft += tempDateLayer.offsetLeft;
            }
            ttop = ttop - tdltop;
            tleft = tleft - tdlleft;
	    }
	...
	dads.left = tleft;
	outObject = th;
	outButton = window.event.srcElement;	//设定外部点击的按钮
...
{ 
	with(window.event)
	{
		...
		try
		{
		 parent.hiddzjsb();  ///关闭近期申报,EditBy:LPF
		}
		catch(e)
		{}
	}
...
{
    setday(document.getElementById(id));
}
