...
function fPopUpCalendarDlg_New(ctrlobj,dirlevel,rowno)
{
	...
	if(N){
	    window.top.captureEvents (Event.CLICK|Event.FOCUS);
            ...
            winPopupWindow.args = ctrlobj;
	        newWINwidth = 202;
            winPopupWindow.win = window.open(dirStr+"js/calendar/masterref/CalendarDlgDateTime.htm","CalendarDialog","dependent=yes,left="+showx + ",top=" + showy + ",width="+newWINwidth + ",height=182px" )
            winPopupWindow.win.focus()
            return winPopupWindow;
	}else{
	    retval = window.showModalDialog(dirStr+"js/calendar/masterref/CalendarDlgDateTime.htm", "", "dialogWidth:197px; dialogHeight:210px; dialogLeft:"+showx+"px; dialogTop:"+showy+"px; status:no; directories:yes;scrollbars:no;Resizable=no; "  );
    }
