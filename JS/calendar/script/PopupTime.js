...
function fPopUpCalendarTimeDlg(ctrlobj)
{
	...
	if(N){
	    window.top.captureEvents (Event.CLICK|Event.FOCUS);
            ...
            winPopupWindow.args = ctrlobj;
	    newWINwidth = 202;
            winPopupWindow.win = window.open("/GWPWEB/resource/calendar/masterref/CalendarDlg.htm","CalendarDialog","dependent=yes,left="+showx + ",top=" + showy + ",width="+newWINwidth + ",height=182px" )
            winPopupWindow.win.focus()
            return winPopupWindow;
	}else{
	    retval = window.showModalDialog("/GWPWEB/resource/calendar/masterref/CalendarDlg.htm", "", "dialogWidth:197px; dialogHeight:210px; dialogLeft:"+showx+"px; dialogTop:"+showy+"px; status:no; directories:yes;scrollbars:no;Resizable=no; "  );
        }
...
function fPopUpCalendarTimeDlg_New(ctrlobj,dirlevel)
{
	if(N){
	    showx = 220 ; // + deltaX;
	    showy = 220; // + deltaY;
	...
	if(N){
	    window.top.captureEvents (Event.CLICK|Event.FOCUS);
            ...
            winPopupWindow.args = ctrlobj;
	    newWINwidth = 202;
            winPopupWindow.win =window.open(dirStr+"js/calendar/masterref/CalendarDlgTime.htm","CalendarDialog","dependent=yes,left="+showx + ",top=" + showy + ",width="+newWINwidth + ",height=182px" )
            winPopupWindow.win.focus()
            return winPopupWindow;
	}else{
	    retval = window.showModalDialog(dirStr+"js/calendar/masterref/CalendarDlgTime.htm", "", "dialogWidth:215px; dialogHeight:100px; dialogLeft:"+showx+"px; dialogTop:"+showy+"px; status:no; directories:yes;scrollbars:no;Resizable=no; "  );
        }
        
	if( retval != null ){
		//document.all(ctrlobj).value=retval + ":00.000";
...
function getDateTime(elName,dirlevel){
	//fPopUpCalendarDlg(document.getElementsByName(elName)[0]);
   
    	fPopUpCalendarTimeDlg_New(elName,dirlevel);
}
