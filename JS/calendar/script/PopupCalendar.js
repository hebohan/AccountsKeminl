...
function CalendarInit()				///Create panel
{
    ...
    htmlAll = "";
    htmlAll += "<iframe style='position:absolute;z-index:1;width:expression(this.nextSibling.offsetWidth);height:expression(this.nextSibling.offsetHeight);top:expression(this.nextSibling.offsetTop);left:expression(this.nextSibling.offsetLeft);' frameborder='0'></iframe>";
    htmlAll += "<div id='" + this.sDIVID + "' style='z-index:2;display:none;position:absolute;width:" + this.Width + ";border:" + this.divBorderCss + ";padding:2px;background-color:#FFFFFF'>";
...
function getDateString(oInputSrc, oInstance) {
    if (oInputSrc && oInstance) {
        ...
        //start modify by liy for DIV靠边时有部分显示不出来
        ...
        var docWidth = document.forms[0].offsetWidth; //页面的可用宽度

        var divLeft = parseInt(CalendargetPos(oInputSrc, "Left"));
        var divWidth = parseInt(CalendarDiv.style.width);

        CalendarDiv.style.pixelLeft = getDivPixelLocation(divLeft, divWidth, docWidth);
        CalendarDiv.style.pixelTop = CalendargetPos(oInputSrc, "Top") + oInputSrc.offsetHeight;
        //end modify by liy for DIV靠边时有部分显示不出来
        CalendarDiv.style.display = (CalendarDiv.style.display == "none") ? "" : "none";
    }
}
//start add by liy for 根据div原位置，宽/高，页面的宽/高 判断 div应处的位置
function getDivPixelLocation(divLocation, divRate, docSize) {
    if (divLocation + divRate > docSize) {
        return docSize - divRate;
    }
    else
        return divLocation;
}
//end add by liy for 根据div原位置，宽/高，页面的宽/高 判断 div应处的位置
...
function hideIframe() {
    if (document.all.ShowReport != null) {
        if (document.all.ShowReport.style.display == 'none') {
            document.all.ShowReport.style.display = '';
        }
        else {
            document.all.ShowReport.style.display = 'none';
        }
    }
}
