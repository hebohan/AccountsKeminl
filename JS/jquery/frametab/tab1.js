...
function TabView(option) {
    ...
    var tabTemplate = '<table class="tab_item"  id="{0}" border="0" cellpadding="0" cellspacing="0"><tr>' + '<td class="tab_item1"></td>'
			...
			+ '</tr></table>';
    var tabContainerTemplate = '<div class="benma_ui_tab" id="{0}"></div>';
    var page = '<iframe id="{0}" frameborder="0" style="WIDTH: 100%;HEIGHT: expression(document.body.clientHeight-60)" scrolling="no" src="{1}" ></iframe>';
...
//                tab_item3.removeClass("tab_item3_mouseover" + bottom);
    ...
    function activate(id, isAdd) {
       ...
       // alert(tab_context.current_page);
    ...
    this.getActive = function() {
        return $(tab_context.current).attr("id");
    }
    ...
    this.add = function(option) {
        ...
        $.extend(op1, option);
        if (op1.title.length > 50) {
            op1.title = op1.title.substring(0, 50);
        }
