...
function fPopUpCalendarDlg_New(ctrlobj)
{
	...
	if(N){
	    window.top.captureEvents (Event.CLICK|Event.FOCUS);
            ...
            winPopupWindow.args = ctrlobj;
	    newWINwidth = 202;
            winPopupWindow.win = window.open("/GWPWEB/resource/calendar/masterref/CalendarDlgTime.htm","CalendarDialog","dependent=yes,left="+showx + ",top=" + showy + ",width="+newWINwidth + ",height=182px" )
            winPopupWindow.win.focus()
            return winPopupWindow;
	}else{
	    retval = window.showModalDialog("/GWPWEB/resource/calendar/masterref/CalendarDlgTime.htm", "", "dialogWidth:197px; dialogHeight:230px; dialogLeft:"+showx+"px; dialogTop:"+showy+"px; status:no; directories:yes;scrollbars:no;Resizable=no; "  );
        }
