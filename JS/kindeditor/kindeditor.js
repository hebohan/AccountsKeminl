...
KE.setting = {
	...
	cssPath: KE.scriptPath + 'P_CSS.css',
	...
	htmlTags : {
		...
		img: ['name', 'src', 'width', 'height', 'border', 'alt', 'title', 'align', 'style', 'onclick', '/'],
		hr : ['class', '/'],
		br : ['/'],
	...
	this.show = function() {
		...
		if (typeof arg.html != "undefined") {
			...
			if (typeof arg.file == "undefined") {
				...
				iframe.src = arg.file;//KE.g[id].pluginsPath + 
			}
		}
...
KE.create = function(id, mode) {
    ...
    //双击编辑区内图片，弹出修改界面
    var selectImageForWebkitX = function(e) {
        var target = e.srcElement || e.target;
        //选中编辑框内img标签，显示相对于占位符属性
        if (target.tagName.toLowerCase() == 'img') {
            var id_Value = target.id;
            if (typeof (id_Value) != "undefined" && id_Value != "") {
                var hd_CurrentContractID = document.getElementById("hd_CurrentContractID");
                var hd_ContractType = document.getElementById("hd_ContractType");
                var hd_FamilyID = document.getElementById("hd_FamilyID");
                var pbURL = null;
                if (typeof (hd_FamilyID) != "undefined" && hd_FamilyID != null && hd_FamilyID != "") {
                    pbURL = '../Contract_Define_Manage/ConditionEdit_Page.aspx?RootNodeID=' + id_Value + '&ContractType=' + hd_ContractType.value + '&FamilyID=' + hd_FamilyID.value + '&a=' + Math.random(); 
                }
                else {
                    pbURL = '../Contract_Define_Manage/ConditionEdit_Page.aspx?RootNodeID=' + id_Value + '&ContractType=' + hd_ContractType.value + '&a=' + Math.random();                
                }
                var returnDatas = window.showModalDialog(pbURL, "", "dialogHeight:400px;dialogWidth:1100px;center:yes;status:no");

                if (typeof (returnDatas) != "undefined" && returnDatas != null && returnDatas != "") {
                    //创建修改后的img标签
                    var dataArray = returnDatas.split('-', 2);
                    var newImgID = dataArray[0]; //占位符代表节点的ID
                    var newImg_Width = dataArray[1]; //占位符长度
                    var imgStrStart = "<img ";
                    var idValue = "id = '" + newImgID + "' height = '16px' width = '" + newImg_Width + "px'";
                    var imgStrEnd = " src='../..Images/underline.gif' />";
                    var imgValue = imgStrStart + idValue + imgStrEnd;
                    
                    //clearEditor("txtContent");
                    //insertHtml("txtContent", newData);
                    KE.util.focus("txtContent"); //将txtContent编辑器设为焦点
                    KE.util.selection("txtContent");
                    KE.util.insertHtml("txtContent", imgValue);
                    
                    //触发服务器控件事件
                    //var Img_Old_ID = document.getElementById("Img_Old_ID");
                    //var Img_New_ID = document.getElementById("Img_New_ID");
                    //Img_Old_ID.value = id_Value;
                    //Img_New_ID.value = newImgID;
                    //__doPostBack("ddl_Event", "");
                }
            }
        }
    };
    KE.event.add(iframeDoc, 'click', selectImageForWebkit);
    KE.event.add(iframeDoc, 'dblclick', selectImageForWebkitX); //添加对编辑框内img标签的双击事件 
