<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="DealDetail.aspx.cs" Inherits="Accounts.Web.mobile.AccountsManage.DealDetail" %>
...
<html>
	<head>
		...
		<title>交易详情 - 账单管理系统 Keminl.cn</title>
        ...
        <script type="text/javascript">
            ...
            function saveremark() {
                ...
                var payid = $('#hidpid').val();
                var remark = $('#remark').val();
                $.ajax({
                    type: "post",
                    url: "/tools/do_ajax.ashx?action=saveremark",
                    data: { "userid": userid, "payid": payid, "remark": remark},
                    dataType: "text",
            ...
            function DL_Delete() {
                var id = $('#hiddlid').val();
                $.dialog.confirm("请确定是否删除该条交易记录?", function () {
                    var userid = $('#hiduserid').val();
                    $.ajax({
                        type: "post",
                        url: "/tools/do_ajax.ashx?action=dl_delete",
            ...
            function callsuccess2() {
                window.location.href = "DealList.aspx";
            }
        </script>
	</head>
	<body class="am-background-disable" style="min-width:100%;background-color:white;">
		<header data-am-widget="header" class="am-header am-header-default am-header-fixed">
			...
			<h1 class="am-header-title">
    			<a href="#title-link" style="font-size:16px;padding-top:0px;" >交易详情</a>
  			</h1>
            <%=Menu %>
		</header>
        <form runat="server">
            <div style="padding-top:10px;background-color:white;overflow:hidden;width:100%" id="a_detail" >
                <h1 style="text-align:center;font-size:18px;padding-bottom:10px;padding-top:5px">交易详情</h1>
                <table class="ListSearch hei12" style="margin:0px auto;" >
                    <%=hidcontent.Value %>
                 </table>
                <div style='text-align:center;padding-top:20px;font-size:15px;'>
                    <a href='javascript:void(0);' onclick='DL_Delete();'><input type='button' value='删除该交易记录' class='btnDel' /></a></div>
            </div>
