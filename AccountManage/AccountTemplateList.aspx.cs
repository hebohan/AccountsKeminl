...
namespace Accounts.Web.AccountManage
{
    public partial class AccountTemplateList : BasePage
    {
        ...
        private void BindDataSource(int pageIndex)
        {
            ...
            string sql = "SELECT id*1 as id,name,remark,CASE is_sys WHEN 1 THEN '是' ELSE '否' END is_sys,sort_id FROM AccountTemplate where is_account=0 and Creator='"+GetLoginUser().Userid + "'";
            ...
            select.DataBaseAlias = "common";
            sql = "select count(0) from AccountTemplate where is_account=0 and Creator='"+GetLoginUser().Userid + "'";
            select.CommandText = sql;
