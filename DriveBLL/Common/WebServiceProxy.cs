...
namespace Accounts.BLL.Common
{
    ...
    public class WebServiceProxy : System.Web.Services.WebService
    {
        ...
        /// <summary>                           
        /// 执行代理类指定方法，带out参数，有返回值                                
        /// </summary>                                  
        /// <param   name="methodName">方法名称</param>                           
        /// <param   name="param">参数</param>                              
        /// <returns>object</returns>                                 
        public object ExecuteQuery(string methodName, object[] param, params Type[] types)
        {
            object rtnObj = null;
            ...
            try
            {
                ...
                //调用方法  
                MethodInfo mi = this._typeName.GetMethod(methodName,types);
                if (mi == null)
                {
                    //记录Web服务方法名错误日志代码位置  
                    throw new TypeLoadException("Web服务访问方法名【" + methodName + "】不正确，请检查！");
